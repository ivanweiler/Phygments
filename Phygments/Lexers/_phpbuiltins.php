...
if(php_sapi_name() == 'cli') {
	...
	function get_php_functions()
	{
		global $PHP_FUNCTION_RE, $PHP_MODULE_RE;
        $modules     = [];
		
        foreach(get_php_references() as $file) {
        	$module = '';
			
            foreach(file($file) as $line) {
                if(!$module) {
                	$matches = [];
                	if(preg_match("#$PHP_MODULE_RE#", $line, $matches)) {
                		$module = $matches[1];
                		$modules[$module] = [];
                	}

            	} elseif(strpos($line, '<h2>Table of Contents</h2>')!==false) {
            		$matches = [];
            		if(preg_match_all("#$PHP_FUNCTION_RE#", $line, $matches)) {
                    ...
                    if($module == 'PHP Options/Info') {
                        unset($modules[$module]['main']);
					} elseif($module == 'Filesystem') {
						unset($modules[$module]['delete']);
					}
	...
	function get_php_references()
	{
		global $PHP_MANUAL_URL, $PHP_MANUAL_DIR, $PHP_REFERENCE_GLOB;
		copy($PHP_MANUAL_URL,'./somefile.tar.gz');
		
		$p = new PharData('./somefile.tar.gz');
		$p->decompress();
		unset($p);
		$p = new PharData('./somefile.tar');
		$p->extractTo('./');
		unset($p);
		
		foreach(glob(sprintf("%s%s", $PHP_MANUAL_DIR, $PHP_REFERENCE_GLOB)) as $file) {
			yield $file;
		}
		
		unlink('./somefile.tar.gz');
		unlink('./somefile.tar');
	}
	
	function regenerate($filename, $modules)
	{
		$content = file_get_contents($filename);
		
		$header = strstr($content, '$MODULES = [', true);
		$footer = strstr($content, "if(php_sapi_name() == 'cli') {");

		$f = fopen($filename, 'w');
		fwrite($f, $header);
		//fwrite($f, sprintf('$MODULES = %s', var_export($modules, true)));
		fwrite($f, sprintf('$MODULES = %s;'."\n\n", pprint($modules)));
		fwrite($f, $footer);
		fclose($f);
	}
		...
		if (is_array($var)) {
			if (empty($var)) {
				...
				foreach ($var as $key => $value) {
					$output .= '  ' . ($export_keys ? pprint($key) . ' => ' : '') . pprint($value, '  ') . ",\n";
				}
				$output .= ']';
			}
